...
export class MapComponent implements OnInit {
  ...
  private initMap() {
    ...
    pointer-events: none;">
        <div class="hotel-marker__icon" style="width: 8px;
        height: 8px;
        background-color: #d88583;
        border: 1px solid #d88583;
        border-radius: 50%;
        position: absolute;
        top: 9px;
        left: 9px;
    ...
    const casualBusyHotels = Array(5)
    .fill(this.centroid)
    .map((x) => [
      x[0] + (Math.random() - 0.6) / 10,
      x[1] + (Math.random() - 0.6) / 10,
    ])
    .map((x) => L.marker(x as L.LatLngExpression, { icon: svgIconBusy }))
    .forEach((x) => x.addTo(this.map));
